...
pub fn dump_data_to_file(data: Vec<u8>, file: &Path) -> io::Result<()> {
    let mut f = File::create(file)
        .expect(&*format!("Could not create file {:?}", file));
    let serialized = bincode::serialize(&data).expect(&*format!("Could not serialize data for file {:?}", file));
...
pub fn load_database(file: &Path) -> Database {
    ...
    bincode::deserialize(&*buf).expect(&*format!("could not deserialize database file at {:?}", file))
}