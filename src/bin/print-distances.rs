...
fn main() {
    ...
    let mut subset_kmers_map: HashMap<u32, Vec<u32>> =
        HashMap::from_iter(subset_kmers.into_iter().map(|kmer| (kmer, vec![])));
    ...
    for (kmer, bits_set) in subset_kmers_map
        .into_iter()
        .sorted_by_key(|(kmer, _set_bits)| *kmer)
    {
